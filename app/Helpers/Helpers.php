...
function checkYoutubeUrlValid($url) {
    try {
        $yurl = 'https://www.youtube.com';
        $getUrl = '';
        $channelId = '';
        $dom = HtmlDomParser::file_get_html($url, false, null, 0);

        if (null !== ($dom->findOne('link[itemprop=url]', 0))) {
            $channelId = $dom->findOne('link[itemprop=url]', 0)->href;
           
            $youtubeurl = explode("/channel/", $channelId);
            $getUrl = @$youtubeurl[0];
            $channelId = @$youtubeurl[1];
...
function createUserFreePlan($user_id) {
    ...
    if($user->userType=='influencer'){
        $checkManger = RelationshipManager::where('type','2')->first();
    } else{
        $checkManger = RelationshipManager::where('type','1')->where('plan_id','1')->inRandomOrder()->first();
    }

    $updateUserCredit = DB::table('users')
        ->where('id', $user_id)
        ->update(['credit_balance' => '250','manager_id'=> $checkManger->id]);
}

function updateRelationshipMgr($plan_id,$user_id){
    $user = User::find($user_id);
    if($user->userType=='influencer'){
        $checkManger = RelationshipManager::where('type','2')->first();
    } else{
        $checkManger = RelationshipManager::where('type','1')->where('plan_id',$plan_id)->inRandomOrder()->first();
    }

    $user->manager_id = $checkManger->id;
    $user->save();
}
...
function youtubeData($channelList,$plateform_type) {
    $user = getUser();
    $channel_list =[];
    if(count($channelList) > 0){
        foreach ($channelList as $key => $channel) {
            $channel_association = \DB::table('channel_association')->select('id','internal_channel_id','promotion_price','influ_id','is_default','is_verified')->where('internal_channel_id', $channel->id)->orderBy('id','DESC')->first();
            ...
            $is_invite = 0;
            $invite = \DB::table('invitations')->where('brand_id', $user->id)->where('influ_id', $influ_id)->where('channel_id', $channel->id)->where('plateform_type','1')->first();
            if (!empty($invite)) {
                $is_invite = 1;
            }
...
function instagramData($channelList,$plateform_type) {
    $user = getUser();
    $channel_list =[];
    if(count($channelList) > 0){
        foreach ($channelList as $key => $channel) {
            ...
            $is_whatsup_send = false;
            $whatsappRequest =\DB::table('whats_app_requests')->where('brand_id',$user->id)->where('channel_id',$channel->id)->where('plateform_type','2')->first();
            if ($whatsappRequest !='') {
                $is_whatsup_send = true;
            }
